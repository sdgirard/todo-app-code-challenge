...
- **Persistence: EF Core + SQLite**, going beyond the requirements' "file-based/in-memory is sufficient" minimum — and, in practice, less effort than hand-rolling file-based storage, since migrations/DbContext absorb schema versioning and concurrent-write handling a JSON/CSV store would need built from scratch. `ITodoRepository` wraps `TodoDbContext` directly, no separate Unit of Work layer (DbContext already is one). Migrations auto-apply at startup. SQLite file lives on a Kubernetes PersistentVolume in the home-lab demo deployment (ephemeral container filesystem would lose data on restart). See [`docs/architecture/backend/overview.md#persistence`](docs/architecture/backend/overview.md#persistence).
