...
public class UpdateTodoEndpointRequestValidatorTests
{
    private readonly UpdateTodoEndpoint.RequestValidator _validator = new();
    ...
    {
        var request = new UpdateTodoEndpoint.Request(title, null, null, false);
    ...
    {
        var request = new UpdateTodoEndpoint.Request(new string('a', 201), null, null, false);
    ...
    {
        var request = new UpdateTodoEndpoint.Request(new string('a', 200), null, null, false);
    ...
    {
        var request = new UpdateTodoEndpoint.Request("Buy milk", new string('a', 2001), null, false);
    ...
    // Leaving out the description entirely is fine — it's optional.
    [Fact]
    ...
    // A due date set in the past should be rejected — the "not in the past" rule
    // applies at update time too, evaluated against the current clock.
    [Fact]
    public void Validate_WithPastDueDate_HasValidationError()
    {
        var request = new UpdateTodoEndpoint.Request("Buy milk", null, DateTime.UtcNow.AddDays(-1), false);
    ...
    {
        var request = new UpdateTodoEndpoint.Request("Buy milk", null, DateTime.UtcNow.Date, false);
    ...
    {
        var request = new UpdateTodoEndpoint.Request("Buy milk", null, DateTime.UtcNow.AddDays(7), false);
    ...
    // regardless of what IsCompleted is set to — it has no validation rule of its own.
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void Validate_WithValidRequestAllFieldsPresent_HasNoValidationErrors(bool isCompleted)
    {
        var request = new UpdateTodoEndpoint.Request(
            "Buy milk", "2% or whole", DateTime.UtcNow.AddDays(1), isCompleted);
