...
**Only five handlers for seven requirement-level operations — no `ICompleteTodoCommandHandler`/`IIncompleteTodoCommandHandler`.** Complete/Incomplete dispatch through the existing `IUpdateTodoCommandHandler` instead of getting their own command type; see [`../../features/update-completion-status/spec.md#cqrs`](../../features/update-completion-status/spec.md#cqrs) for how. That makes it the one handler in this feature with two callers, an exception to the "one caller per handler" framing below.
