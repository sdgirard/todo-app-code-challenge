# To-Do App High-level Architecture
...
Containerization is an optional enhancement, not a core requirement, so it's not reflected in the diagram below. If it happens, the app gets built into two separate Docker images (backend and frontend) and pushed to a private Harbor registry — see the container image and Harbor docs under [Related Docs](#related-docs) below.
...
- **ASP.NET Core Minimal API** — endpoint-per-class API (no controllers) exposing REST endpoints under `/todos`. Owns the to-do domain model, request validation, and status codes/error contract. See [`backend/overview.md`](./backend/overview.md) for the full design.
- **Persistence** — EF Core + SQLite behind a repository interface (`ITodoRepository`), going beyond the requirements doc's "file-based or in-memory is sufficient" minimum — and, in practice, less effort than hand-rolling file-based storage, since migrations/DbContext absorb schema versioning and concurrent-write handling a JSON/CSV store would need built from scratch. See [`backend/overview.md#persistence`](./backend/overview.md#persistence) for the full decision and what was ruled out.
...
- **Spec generation (backend):** `Microsoft.AspNetCore.OpenApi` — the built-in .NET OpenAPI generator (introduced in .NET 9, still the built-in option on .NET 10), driven off the Minimal API endpoint metadata (`.WithName()`, `.WithSummary()`, etc. — see [`backend/overview.md#endpoint-pattern`](./backend/overview.md#endpoint-pattern)). No extra dependency (ruled out Swashbuckle; more mature/customizable but an unnecessary dependency here).
...
├── frontend/                # React SPA — see frontend/overview.md
├── docs/
├── scripts/
└── VERSION
```

`backend/` and `frontend/` are siblings at the repo root — matching the "two independently deployable pieces" framing above. See [`backend/overview.md#directory-structure`](./backend/overview.md#directory-structure) for the full `backend/` tree (Gateway + feature-library modular monolith) and [`frontend/overview.md#directory-structure`](./frontend/overview.md#directory-structure) for the full `frontend/` tree.
...
- [`../infra/container-image.md`](../infra/container-image.md) — backend container image design, if containerization happens
- [`../infra/container-image-frontend.md`](../infra/container-image-frontend.md) — frontend container image design, if containerization happens
...
- [`../features/`](../features/) — per-feature specs (backend and frontend), one per CRUD/status operation; see the README's Documentation Map for the full list
