...
- **`GetTrackedByIdAsync` still not generalized, despite a second consumer.** [`../update-todo/spec.md#open-questions`](../update-todo/spec.md#open-questions) flagged a `GetByIdAsync(id, track: bool)` overload (or a shared private helper) as worth reconsidering once a second tracked-fetch consumer showed up; `DeleteTodoCommandHandler` is that second consumer. Left as two separate named methods anyway — the boolean-parameter footgun that spec rejected still applies, and two three-line methods is not yet enough duplication to outweigh the clarity of two distinctly-named calls. Complete/Incomplete landed through path (b), not (a) — see [`../update-completion-status/spec.md#cqrs`](../update-completion-status/spec.md#cqrs) — so it did not add a third direct consumer; this remains open for whenever one actually appears.
- **Whether Complete/Incomplete's specs should link back here.** Resolved: [`../update-completion-status/spec.md`](../update-completion-status/spec.md) is the Complete/Incomplete spec, shipped as a single `PATCH /todos/{id}` endpoint rather than two.
