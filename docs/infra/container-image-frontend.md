...
**The published image doesn't hardcode a backend URL — it's supplied at container startup, not baked in at build time.** Originally missed: `VITE_API_BASE_URL` (see [`../architecture/frontend/overview.md#api-base-url--local-dev-origin`](../architecture/frontend/overview.md#api-base-url--local-dev-origin)) is a *Vite build-time* env var, only read from `.env.development` for `npm run dev` — `docker/Dockerfile.app`'s `npm run build` never had an equivalent value, so the shipped `todo-app-web` image's bundle always requested API paths as bare relative paths (`/todos`, not `https://backend-host/todos`). That's silently correct only when something in front of both images (a single reverse-proxy origin) routes `/todos` to the backend and everything else to the SPA; running the two images standalone (e.g. `docker-compose.yml`, each on its own published port) means the browser's `/todos` fetch hits **nginx**, which has no such route and falls through to the SPA-fallback `location /` block — the frontend gets back `index.html` where it expected JSON, and the app crashes trying to read a list off of it.

**Fix: a small runtime-config file, generated by the container's entrypoint, not the build.**

- `docker/env-config.js.template` — a template for `window.__ENV__ = { API_BASE_URL: "${API_BASE_URL}" }`.
- `docker/docker-entrypoint.sh` — runs `envsubst` against that template into `dist/env-config.js` at container *startup* (using the shell env var actually present in that container, not whatever was true at image-build time), then `exec`s the real `nginx` command (must end in `exec "$@"` for the same PID-1/graceful-shutdown reason as the backend's debug entrypoint — see [`container-image.md#remote-debugging-enable_debug-build-arg`](./container-image.md#remote-debugging-enable_debug-build-arg)).
- `frontend/index.html` loads `/env-config.js` before the app bundle; `frontend/src/api/mutator/fetchWithBaseUrl.ts` reads `window.__ENV__?.API_BASE_URL`, falling back to `import.meta.env.VITE_API_BASE_URL` when it's absent (i.e. `npm run dev`, which never gets a container entrypoint to generate that file).
- **Default (`API_BASE_URL` unset): empty string**, i.e. relative paths — so a deployment that does front both images behind one origin still works with zero configuration; only a split-origin setup needs to set it explicitly (`docker-compose.yml`/`docker-compose.registry.yml` set it to the backend's host-published port, since the *browser* makes the fetch, not the container).

One built image now works against any backend URL without a rebuild — the correct shape for a published, reusable container image, not just the one environment it happened to be tested in first.
...
- Runtime `API_BASE_URL` substitution confirmed: `docker-entrypoint.sh` generates `dist/env-config.js` correctly from the container's env at startup (verified with the var set and unset, both against a fresh build and against the published Harbor image); `nginx` is still PID 1 (`exec "$@"` correctness preserved, same check as the backend's entrypoint). Found via a real `docker-compose.registry.yml` run against a split-origin setup (each image on its own host port) crashing with `Cannot read properties of undefined (reading 'length')` — the frontend was fetching bare `/todos` against nginx itself, getting `index.html` back instead of JSON. See [Runtime API Base URL](#runtime-api-base-url) above.
...
- The demo/production value for `API_BASE_URL` isn't decided — same still-TODO dependency as `VITE_API_BASE_URL`'s production value (see [`../architecture/frontend/overview.md#api-base-url--local-dev-origin`](../architecture/frontend/overview.md#api-base-url--local-dev-origin)): depends on how/where the built SPA and backend actually get served in the home-lab deployment.
