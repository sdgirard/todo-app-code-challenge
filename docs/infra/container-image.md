...
- **Local multi-arch build/push** (outside CI, e.g. testing before a push): `docker buildx build -f docker/Dockerfile.gateway --platform linux/amd64,linux/arm64 -t docker.thecameraeye.ca/inhouse/todo-app-gateway:local --push .` (a `--push` is required for a true multi-platform build — `--load` only works for a single platform at a time, matching the host).
