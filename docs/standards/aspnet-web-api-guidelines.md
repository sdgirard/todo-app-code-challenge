...
- Prefer a hand-written fake over a mocking library for a single-method interface (e.g. `ITodoRepository` in the `AddTodo` handler test) — no dependency needed for something a five-line private class covers just as well. Reach for a mocking library only if/when a fake becomes genuinely tedious to hand-maintain (many methods, complex call verification).
- Integration tests (`TodoApi.Gateway.Tests`) use a real `WebApplicationFactory<Program>` against SQLite's in-memory mode (`Data Source=:memory:`, kept-open connection swapped in via `ConfigureWebHost`) — not EF Core's `InMemory` provider (already ruled out for non-test use, see [`backend/overview.md#persistence`](../architecture/backend/overview.md#persistence); the same reasoning against unrealistic relational semantics applies to tests). Each test class gets its own isolated in-memory database via a fresh `WebApplicationFactory` instance.
